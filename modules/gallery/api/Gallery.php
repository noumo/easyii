...
class Gallery extends \yii\easyii\components\API
{
    ...
    public function api_get($id_slug)
    {
        if(!isset($this->_albums[$id_slug])) {
            $this->_albums[$id_slug] = $this->findAlbums($id_slug);
        }
    ...
    private function findAlbums($id_slug)
    {
        $files = Album::find()->where(['or', 'file_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])->one();

        return $files ? new AlbumObject($files) : null;
    }
}